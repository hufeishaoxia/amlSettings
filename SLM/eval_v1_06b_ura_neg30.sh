#!/bin/bash
# Wait for the v1 0.6B URA neg30 training to finish, then eval all checkpoints
# with eval_auc.py and print a summary.
set -euo pipefail
cd "$(dirname "$0")"
export NCCL_IB_DISABLE=1

MODEL_DIR=${MODEL_DIR:-"output/v1_Qwen3-0.6B_ura_neg30_ep3"}
...
echo "[$(date)] Waiting for v1 0.6B (neg30) training to finish..."
while pgrep -f "train\.py.*${MN}" > /dev/null 2>&1 \
   || pgrep -f "torchrun.*train\.py.*Qwen3-0.6B.*neg30" > /dev/null 2>&1; do
    sleep 30
done
