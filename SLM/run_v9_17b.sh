#!/bin/bash
# v9 with Qwen/Qwen3-1.7B — same recipe as v9 (Qwen3-0.6B) but bigger model.
# data_v9 JSONL, all traffic, neg_frac=0.3, cl=4096, 3 epochs, lr=2e-5, bs=128.
# Use micro_batch=1 + grad_ckpt to fit 1.7B at cl=4096 on A100.
set -euo pipefail
cd "$(dirname "$0")"
...
export MICRO_BATCH_SIZE=${MICRO_BATCH_SIZE:-1}          # 1 for 1.7B at cl=4096
...
export OUTPUT_DIR=${OUTPUT_DIR:-"output/v9_Qwen3-1.7B_all_ep${NUM_EPOCHS}"}
